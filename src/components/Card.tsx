...
const Card: React.FC<Album> = ({ album, artist, cover, discogsId }) => {
  const isSkeletonLoading = !discogsId || discogsId.includes("skeleton-");
  return (
    <li
      className={isSkeletonLoading ? "card card--loading" : "card"}
      key={discogsId}
    >
      <Link
        className="card__wrapper link--reset"
        to={isSkeletonLoading ? "/albums" : `/albums/album/${discogsId}`}
      >
        <Cover
          offsetVertical={768}
          cover={cover}
